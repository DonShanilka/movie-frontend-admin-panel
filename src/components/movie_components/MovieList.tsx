...
export default function MovieList() {
  ...
  const openUpdateModal = (movie: MovieTypes) => {
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-900">
            {movies.map((movie: MovieTypes) => (
              <tr key={movie.id} className="hover:bg-gray-900 transition">
                <td className="px-3 py-2">
                  <img
                    src={movie.Banner ? `data:image/jpeg;base64,${movie.Banner}` : "/placeholder.png"}
                    alt={movie.Title}
                ...
                <td className="px-3 py-2 text-center">
                  <a href={movie.Movie_URL} target="_blank" className="text-blue-400 hover:underline">Play</a>
                </td>
                <td className="px-3 py-2">
